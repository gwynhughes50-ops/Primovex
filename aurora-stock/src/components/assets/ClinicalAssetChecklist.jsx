...
function readinessCopy(status) {
  if (status === "critical") {
    return {
      ...
      icon: AlertTriangle,
      badge: "border-destructive/40 bg-destructive/15 text-destructive-foreground",
...
function itemStatusClass(status, expiryDays) {
  ...
  if (status === "N/A") return "border-border bg-muted text-muted-foreground";
  return "border-emerald-500/30 bg-emerald-500/10 text-emerald-100";
}

function StatCard({ label, value, detail, icon: Icon }) {
  return (
    <div className="rounded-2xl border border-border bg-card/70 p-4 text-card-foreground shadow-sm">
      <div className="flex items-center justify-between gap-3">
        ...
        {Icon && (
          <div className="rounded-2xl border border-border bg-background/50 p-3 text-primary">
...
<body>
  ...
  if (loading) {
    return <div className="rounded-2xl border border-border bg-card p-5 text-card-foreground">Loading {entityLabelPlural}...</div>;
  }
    ...
    return (
      <div className="rounded-3xl border border-border bg-card p-6 text-card-foreground shadow-sm">
        <div className="max-w-3xl">
          ...
          <h2 className="mt-4 text-2xl font-semibold tracking-tight text-foreground">No {entityLabelPlural} found</h2>
  ...
  return (
    <div className="space-y-5 text-foreground">
      <section className={classNames("relative overflow-hidden rounded-3xl border border-border bg-card p-5 shadow-sm", `bg-gradient-to-br ${statusMeta.ring}`)}>
        <div className="absolute right-6 top-6 hidden rounded-full border border-border/80 bg-background/40 p-8 opacity-40 md:block">
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            ...
            {selected && (
              <div className="mt-4 flex flex-wrap gap-2 text-xs text-muted-foreground">
                ...
                <span className="inline-flex items-center gap-1 rounded-full border border-border bg-background/50 px-3 py-1">
          ...
          <div className="grid min-w-[260px] grid-cols-2 gap-3">
            <div className="rounded-3xl border border-border bg-background/60 p-4 text-center shadow-sm">
              <div className="text-4xl font-bold tracking-tight text-foreground">{readiness.score}%</div>
              <div className="mt-1 text-xs uppercase tracking-[0.18em] text-muted-foreground">Readiness</div>
            </div>
            <div className={classNames("rounded-3xl border p-4 text-center shadow-sm", statusMeta.badge)}>
        ...
        <div className="space-y-5">
          <div className="rounded-3xl border border-border bg-card p-4 text-card-foreground shadow-sm">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                <label className="text-sm font-medium text-muted-foreground">{capitalise(entityLabel)}</label>
                <select
                  className="min-w-[260px] rounded-xl border border-input bg-background px-3 py-2 text-sm text-foreground outline-none ring-offset-background focus:ring-2 focus:ring-ring"
                  value={selectedId}
          ...
          <div className="rounded-3xl border border-border bg-card text-card-foreground shadow-sm">
            <div className="border-b border-border px-5 py-4">
              <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Tick through the expected contents. Individual vials may not have a barcode once removed from the original packaging, so this check belongs to the asset QR code.</p>
                </div>
                <div className="rounded-full border border-border bg-background px-3 py-1 text-xs text-muted-foreground">{monthKey()}</div>
              </div>
            </div>
                  ...
                  <div key={item.id} className="p-4 transition hover:bg-muted/30">
                    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_180px_120px_160px_160px] lg:items-start">
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          {enableSections && item.section && (
                            <span className="rounded-full border border-border bg-background px-2.5 py-1 text-xs text-muted-foreground">{item.section}</span>
                          )}
                        ...
                        <textarea
                          className="mt-3 min-h-[66px] w-full rounded-xl border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground outline-none focus:ring-2 focus:ring-ring"
                      ...
                      <div>
                        ...
                        <select
                          className="mt-2 w-full rounded-xl border border-input bg-background px-3 py-2 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring"
                          value={status}
                      ...
                      <div>
                        <label className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">Qty</label>
                        <input
                          className="mt-2 w-full rounded-xl border border-input bg-background px-3 py-2 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring"
                          value={result.qty || ""}
                      ...
                      <div>
                        <label className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">Batch / Serial</label>
                        <input
                          className="mt-2 w-full rounded-xl border border-input bg-background px-3 py-2 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring"
                          value={result.batch || ""}
                      ...
                      <div>
                        <label className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">Expiry</label>
                        <input
                          className="mt-2 w-full rounded-xl border border-input bg-background px-3 py-2 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring"
                          value={result.expiry || ""}
            ...
            <div className="border-t border-border p-5">
              <label className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">Overall notes</label>
              <textarea
                className="mt-2 min-h-[90px] w-full rounded-xl border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground outline-none focus:ring-2 focus:ring-ring"
                placeholder="Overall notes, missing item action, replacement ordered, or reason for delay..."
                value={form.notes}
                onChange={(event) => setForm((previous) => ({ ...previous, notes: event.target.value }))}
              />
              <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-xs text-muted-foreground">Saved checks are audit records. Use notes to explain any missing, expired or replaced items.</p>
                <button
                  type="button"
                  onClick={submit}
        ...
        <aside className="space-y-5">
          <div className="rounded-3xl border border-border bg-card p-5 text-card-foreground shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-foreground">Asset QR Label</h3>
                <p className="text-xs text-muted-foreground">Retrofit physical kits with a MedTrak identity.</p>
              </div>
            </div>
            <div className="mt-4 rounded-2xl border border-border bg-background p-4 text-center">
              <img
                className="mx-auto h-36 w-36 rounded-xl border border-border bg-white p-2"
              ...
              />
              <div className="mt-3 font-mono text-sm font-semibold text-foreground">{medtrakAssetId}</div>
              <div className="mt-1 text-xs text-muted-foreground">{selected?.location || "No location set"}</div>
            </div>
            <button
              type="button"
              onClick={() => openAssetLabelPrintWindow({ asset: selected, collectionName, title: checklistTitle })}
              className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl border border-border bg-background px-4 py-2.5 text-sm font-semibold text-foreground transition hover:bg-muted"
            >
              <Printer className="h-4 w-4" /> Print / save label
            </button>
            <p className="mt-3 text-xs leading-5 text-muted-foreground">The manufacturer barcode remains useful for boxed stock. Once a kit is assembled or split into individual vials, use this MedTrak QR code to identify the kit and verify contents.</p>
          </div>

          <div className="rounded-3xl border border-border bg-card p-5 text-card-foreground shadow-sm">
            <h3 className="font-semibold text-foreground">MedAI readiness notes</h3>
