...
export default function Dashboard() {
  ...
  const [dashboardPreferences, setDashboardPreferences] = useState(() =>
    normaliseDashboardPreferences(profile?.dashboardPreferences, role)
  );

  useEffect(() => {
    setDashboardPreferences(normaliseDashboardPreferences(profile?.dashboardPreferences, role));
  }, [profile?.dashboardPreferences, role]);

  const visibleWidgets = useMemo(() => new Set(dashboardPreferences.visible), [dashboardPreferences.visible]);

  async function persistDashboardPreferences() {
    setDashboardSaving(true);
    try {
      await saveDashboardPreferences(user?.uid, dashboardPreferences);
      setCustomiseOpen(false);
    } catch (error) {
      console.error("Could not save dashboard preferences", error);
      alert("Dashboard saved on this device, but could not sync to your profile.");
      setCustomiseOpen(false);
    } finally {
  ...
  const widgetContent = {
    ...
    quickActions: (
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="text-xl font-semibold text-[color:var(--medtrak-text)]">Home</div>
          <div className="text-sm text-[color:var(--medtrak-muted)]">Your personal view of what matters today.</div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <ThemePickerButton />
          <Button variant="outline" onClick={() => setCustomiseOpen(true)} className="rounded-xl">
            <Settings2 className="mr-2 h-4 w-4" /> Customise Home
          </Button>
          <Button
            onClick={() => { resetUseModal(); setUseOpen(true); }}
            className="rounded-2xl px-5 py-5 text-sm font-semibold bg-gradient-to-r from-teal-500 to-emerald-400 text-slate-950 shadow-lg shadow-emerald-500/20"
          >
            <Barcode className="mr-2 h-4 w-4" /> Use stock
          </Button>
        </div>
      </div>
    ),
    stockSummary: (
      <section className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <Card className={`${cardBase} p-4`}>
          <p className={`text-[0.7rem] font-medium uppercase tracking-wide ${muted}`}>Total items</p>
          <p className="mt-1 text-3xl font-semibold text-slate-50">{stockLoading ? "—" : totalItems}</p>
        ...
        <Card className="medtrak-low-stock-card rounded-2xl p-4">
          <div className="flex items-center justify-between"><p className="text-[0.7rem] font-medium uppercase tracking-wide text-amber-100/90">Low stock</p><AlertTriangle className="h-4 w-4 text-amber-200" /></div>
          <p className="mt-1 text-3xl font-semibold text-amber-50">{stockLoading ? "—" : lowStockItems}</p>
        ...
        <Card className="medtrak-temperature-card rounded-2xl p-4">
          <div className="flex items-center justify-between"><p className="text-[0.7rem] font-medium uppercase tracking-wide text-sky-100/90">Temperature</p><Thermometer className="h-4 w-4 text-sky-200" /></div>
          <p className="mt-1 text-3xl font-semibold text-sky-50">{tempStatus.headline}</p>
          <p className={`mt-1 text-xs ${tempStatus?.within === false ? "text-rose-200" : "text-sky-100/80"}`}>{tempStatus.sub}</p>
        </Card>
      </section>
    ),
    stockActivity: (
      <Card className={`${cardBase} p-4`}>
        <div className="mb-3 flex items-center justify-between">
          <div><p className={`text-xs font-medium uppercase tracking-wide ${muted}`}>Recent stock activity</p><p className={`text-sm ${sub}`}>Latest movements recorded.</p></div>
          <Button variant="ghost" onClick={() => navigate("/inventory")} className="text-xs rounded-full px-3">View inventory</Button>
        </div>
        <div className="space-y-2 text-xs text-slate-300">
          {movesLoading ? <p className="text-slate-400">Loading activity…</p> : recentMoves.length === 0 ? <p className="text-slate-400">No recent movements yet.</p> : recentMoves.map((m) => <p key={m.id}>{formatMove(m)}</p>)}
        </div>
      </Card>
    ),
    issues: (
      <Card className={`${cardBase} p-4`}>
        <div className="mb-3 flex items-center justify-between">
          <div><p className={`text-xs font-medium uppercase tracking-wide ${muted}`}>Warnings</p><p className={`text-sm ${sub}`}>Only exceptions that may need attention.</p></div>
          <Package className="h-4 w-4 text-slate-400" />
        </div>
        <div className="space-y-2 text-xs">
          {lowLoading || expLoading || tempLoading ? (
            <div className="rounded-xl border border-slate-700/80 bg-slate-950/40 px-3 py-2 text-slate-200">Loading issues…</div>
          ) : issues.length === 0 ? (
            <div className="rounded-xl border border-emerald-400/25 bg-emerald-500/10 px-3 py-2 text-emerald-100">No issues detected.</div>
          ) : issues.map((c) => {
            const tone = c.tone === "rose" ? "border-rose-500/40 bg-rose-500/10 text-rose-50" : c.tone === "amber" ? "border-amber-400/40 bg-amber-500/10 text-amber-50" : "border-slate-700/80 bg-slate-950/40 text-slate-200";
            return <div key={c.key} className={`rounded-xl border px-3 py-2 ${tone}`}>{c.text}</div>;
          })}
        </div>
        <div className="mt-3 flex gap-2"><Button variant="ghost" className="text-xs rounded-full px-3" onClick={() => navigate("/inventory")}>Inventory</Button><Button variant="ghost" className="text-xs rounded-full px-3" onClick={() => navigate("/temperature")}>Temperature</Button></div>
      </Card>
    ),
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Primovex Home</p>
          <h1 className="mt-1 text-2xl font-semibold text-[color:var(--medtrak-text)]">A calm view of your practice</h1>
        </div>
        {!visibleWidgets.has("quickActions") && (
          <Button variant="outline" onClick={() => setCustomiseOpen(true)} className="rounded-xl"><Settings2 className="mr-2 h-4 w-4" /> Customise</Button>
        )}
      </div>
      ...
      {dashboardPreferences.order.map((id) => visibleWidgets.has(id) ? <React.Fragment key={id}>{widgetContent[id]}</React.Fragment> : null)}
