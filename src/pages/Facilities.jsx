...
export default function Facilities() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener('primovex:equipment-registry-changed', refresh);
    };
  }, []);
  ...
  return (
    <div className={`space-y-5 ${text}`}>
      <section className={`${panel} overflow-hidden p-5 sm:p-7`}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-semibold sm:text-3xl">Shared spaces and building readiness</h1>
            <p className={`mt-2 max-w-3xl text-sm ${muted}`}>Facilities reads the shared Space Registry used by Admin, Sense and Primovex Mobile. Cleaning, equipment, maintenance and Smart Tags remain attached to the same permanent space identity.</p>
          </div>
      ...
      {tab === 'spaces' && (
        <SpaceBuilder state={spaceState} commit={commitSpaces} actor={actor} onSelect={setSelectedRoomId} />
      )}

      {(tab === 'overview' || tab === 'spaces') && (
        <section className={`${panel} p-4 sm:p-5`}>
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div><h2 className="text-lg font-semibold">Shared Space Registry</h2><p className={`text-sm ${muted}`}>Spaces created in Practice Administration appear here automatically. Open one to clean it, review equipment, or report an issue.</p></div>
      ...
      {selectedEquipment && <div className="fixed inset-0 z-[95] flex items-end justify-center bg-black/60 p-0 sm:items-center sm:p-6" onClick={()=>setSelectedEquipmentId(null)}><div className={`${panel} w-full max-w-lg rounded-b-none p-5 sm:rounded-2xl`} onClick={(e)=>e.stopPropagation()}><div className="flex items-start justify-between"><div><p className={`text-xs font-semibold ${muted}`}>{selectedEquipment.id}</p><h2 className="mt-1 text-xl font-semibold">{selectedEquipment.name}</h2><p className={`text-sm ${muted}`}>{selectedEquipment.category}</p></div><button onClick={()=>setSelectedEquipmentId(null)} className="rounded-full p-2 hover:bg-white/5"><X className="h-5 w-5"/></button></div><div className="mt-5"><label className={`text-xs ${muted}`}>Current recorded room</label><select value={selectedEquipment.roomId} onChange={(e)=>moveEquipment(selectedEquipment.id,e.target.value)} className="mt-1 w-full rounded-xl border border-[color:var(--medtrak-border)] bg-[color:var(--medtrak-panel)] px-3 py-3">{state.rooms.map((room)=><option key={room.id} value={room.id}>{room.name}</option>)}</select><p className={`mt-2 text-xs ${muted}`}>Changing the room creates a timestamped movement record under the signed-in user.</p></div><div className="mt-5 rounded-xl border border-[color:var(--medtrak-border)] p-3"><span className={`text-xs ${muted}`}>Assigned to</span><div className="mt-1 text-sm font-medium">{selectedEquipment.assignment?.assignedToName || 'Unassigned'}</div><div className={`text-xs capitalize ${muted}`}>{selectedEquipment.assignment?.type === 'loan' ? `On loan${selectedEquipment.assignment?.loanDueDate ? ` · return ${formatShort(selectedEquipment.assignment.loanDueDate)}` : ''}` : selectedEquipment.assignment?.type === 'permanent' ? 'Permanent assignment' : 'No personal assignment'}</div>{selectedEquipment.assignment?.notes && <p className={`mt-2 text-xs ${muted}`}>{selectedEquipment.assignment.notes}</p>}</div><div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-xl border border-[color:var(--medtrak-border)] p-3"><span className={`text-xs ${muted}`}>Last seen</span><div className="mt-1 text-sm font-medium">{formatDate(selectedEquipment.lastSeenAt)}</div><div className={`text-xs ${muted}`}>{selectedEquipment.lastSeenBy}</div></div><div className="rounded-xl border border-[color:var(--medtrak-border)] p-3"><span className={`text-xs ${muted}`}>Service due</span><div className="mt-1 text-sm font-medium">{formatShort(selectedEquipment.serviceDue)}</div><span className={`text-xs ${muted}`}>PAT: {formatShort(selectedEquipment.patDue)}</span></div></div></div></div>}
      {showEquipmentWizard && <EquipmentRegistrationWizard spaces={spaceState.spaces} actor={actor} connectedDevices={connectedDevices} onClose={()=>setShowEquipmentWizard(false)} onSaved={(equipmentId)=>{setShowEquipmentWizard(false);setState(loadFacilitiesState());setSelectedEquipmentId(equipmentId);}} />}
    </div>
  );
}
