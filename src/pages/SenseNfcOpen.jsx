...
export default function SenseNfcOpen() {
  ...
  const entity = entityType === 'asset' ? state.assets.find((x) => x.id === entityId) : state.spaces.find((x) => x.id === entityId);
  const facilities = useMemo(() => loadFacilitiesState(), []);
  const readiness = entityType === 'space' && entity ? calculateSpaceReadiness(entity, state, facilities) : null;

  useEffect(() => {
    if (entityType === 'space' && entity) saveSenseState(confirmSpaceContext(state, entity.id, displayName || user?.email || 'Signed-in user', 'nfc', 1));
  }, []);
