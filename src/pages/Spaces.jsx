...
export default function Spaces() {
  ...
  return (
    <div className="space-y-5 text-[color:var(--medtrak-text)]">
      <section className={`${panel} overflow-hidden p-5 sm:p-7`}>
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className={`mt-2 max-w-3xl text-sm ${muted}`}>Create the digital identity for each room, write its Smart Tag and see the same active-room context across desktop and mobile.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button onClick={() => document.getElementById('sense-tag-manager')?.scrollIntoView({ behavior: 'smooth' })} className={`${button} bg-[color:var(--medtrak-accent)] text-white`}><Radio className="mr-2 inline h-4 w-4" />Set up room tag</button><button onClick={runMockScan} disabled={scanning} className={button}><ScanLine className="mr-2 inline h-4 w-4" />{scanning ? 'Scanning…' : 'Test room detection'}</button>
      ...
      <div className="grid gap-5 xl:grid-cols-[0.85fr_1.5fr]">
        ...
        {selectedSpace && readiness && (
          <section className={`${panel} p-4 sm:p-5`}>
            ...
            <div className="mt-5"><h3 className="font-semibold">Assets currently here</h3><div className="mt-2 grid gap-2 sm:grid-cols-2">{assetsHere.map((asset) => <div key={asset.id} className="rounded-2xl border border-[color:var(--medtrak-border)] p-3"><div className="flex items-start justify-between gap-3"><span><b className="block">{asset.name}</b><small className={muted}>Home: {senseState.spaces.find((space) => space.id === asset.homeSpaceId)?.name || asset.homeSpaceId}</small></span>{asset.currentSpaceId !== asset.homeSpaceId && <StatusPill tone="warning">Away from home</StatusPill>}</div><select value={asset.currentSpaceId} onChange={(event) => commit(moveAsset(senseState, asset.id, event.target.value, actor))} className="mt-3 w-full rounded-xl border border-[color:var(--medtrak-border)] bg-[color:var(--medtrak-bg)] px-3 py-2 text-sm">{activeSpaces.map((space) => <option key={space.id} value={space.id}>{space.name}</option>)}</select></div>)}</div></div>
      ...
      <NfcManager state={senseState} commit={commit} actor={actor} defaultEntityId={selectedSpaceId} />

      <section className={`${panel} p-4 sm:p-5`}>
        <div className="flex items-center gap-3"><ShieldCheck className="h-5 w-5 text-[color:var(--medtrak-accent)]" /><div><h2 className="font-semibold">Sense Foundation boundary</h2><p className={`text-sm ${muted}`}>Room identity can be written to NFC tags now. Presence sessions are shared through Firestore, while room registry data remains compatible with the existing Sense model.</p></div></div>
      </section>
    </div>
  );
}
