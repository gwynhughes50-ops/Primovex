...
import { cancelGovernedProposal, completeGovernedProposal, confirmGovernedProposal, parseGovernedProposal } from "@/orb/GovernedActionStore";
import {
  buildAssetQrPayload,
...
export default function ClinicalAssetChecklist({
  ...
  const [activeOrbAction, setActiveOrbAction] = useState(null);
  ...
  useEffect(() => {
    if (orbProposalState.proposal?.status === 'confirmed') {
      setActiveOrbAction(orbProposalState.proposal);
      setSelectedId(orbProposalState.proposal.entityId);
    }
  }, [orbProposalState.proposal]);

  useEffect(() => {
    const requestedAsset = new URLSearchParams(location.search).get('asset');
    if (requestedAsset && entities.some((entity) => entity.id === requestedAsset)) setSelectedId(requestedAsset);
  }, [entities, location.search]);

  function clearOrbActionParams() {
    const params = new URLSearchParams(location.search);
    ['orbAction', 'orbActionId', 'orbKind', 'orbCollection', 'orbEntityId', 'orbEntityName', 'orbCreatedAt', 'orbExpiresAt'].forEach((key) => params.delete(key));
    navigate(`${location.pathname}?${params.toString()}`, { replace: true });
  }

  function confirmOrbAction() {
    setOrbActionMessage("");
    if (!can('inventory.verify')) {
      setOrbActionMessage('Your role cannot confirm clinical inventory verification actions.');
      return;
    }
    const proposal = orbProposalState.proposal;
    if (!proposal || !entities.some((entity) => entity.id === proposal.entityId)) {
      setOrbActionMessage('The proposed clinical asset is unavailable. Ask Orb to prepare a new proposal.');
      return;
    }
    try {
      const confirmed = confirmGovernedProposal(proposal, orbActor);
      setActiveOrbAction(confirmed);
      setSelectedId(confirmed.entityId);
      setOrbActionMessage('Proposal confirmed. Complete every checklist item, then save the verification.');
    } catch (error) { setOrbActionMessage(error?.message || 'Orb proposal could not be confirmed.'); }
  }

  function cancelOrbAction() {
    cancelGovernedProposal(orbProposalState.proposal || activeOrbAction, orbActor);
    setActiveOrbAction(null);
    setOrbActionMessage('Orb reconciliation proposal cancelled.');
    clearOrbActionParams();
  }
  ...
  async function submit() {
    ...
    try {
      ...
      const payload = {
        ...
        orbAction: activeOrbAction ? { id: activeOrbAction.id, type: activeOrbAction.type, confirmedAt: activeOrbAction.confirmedAt } : null,
      };
      const checkRef = await createCheck(collectionName, selected.id, payload);
      if (activeOrbAction) {
        try {
          completeGovernedProposal(activeOrbAction.id, checkRef?.id || null, orbActor);
        } catch (auditError) {
          console.error('Orb governed-action audit completion failed after checklist save:', auditError);
          setOrbActionMessage('The verification was saved, but its Orb audit status needs management review.');
        }
        setActiveOrbAction(null);
        clearOrbActionParams();
      }
...
<body>
  ...
  return (
    <div className="space-y-5 text-[color:var(--medtrak-text)]">
      {(orbProposalState.proposal || orbProposalState.error || activeOrbAction) && (
        <section className="rounded-2xl border border-primary/35 bg-primary/10 p-4" aria-label="Orb governed reconciliation">
          <div className="flex items-start gap-3">
            <ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-primary" />
            <div className="min-w-0 flex-1">
              <h3 className="font-semibold">Orb governed reconciliation</h3>
              {orbProposalState.error ? <p className="mt-1 text-sm text-rose-500">{orbProposalState.error}</p> : <p className="mt-1 text-sm text-[color:var(--medtrak-muted)]">{activeOrbAction ? `Confirmed for ${activeOrbAction.entityName}. No readiness result is written until the completed checklist is saved.` : `Review the proposal for ${orbProposalState.proposal?.entityName}. Confirmation starts the checklist only; it does not mark the asset ready or change stock.`}</p>}
              {orbActionMessage && <p className="mt-2 text-sm font-medium">{orbActionMessage}</p>}
              <div className="mt-3 flex flex-wrap gap-2">
                {!activeOrbAction && orbProposalState.proposal && <button type="button" onClick={confirmOrbAction} className="rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground">Confirm and begin checklist</button>}
                {(orbProposalState.proposal || activeOrbAction) && <button type="button" onClick={cancelOrbAction} className="rounded-xl border border-[color:var(--medtrak-border)] px-4 py-2 text-sm font-semibold">Cancel proposal</button>}
                {orbProposalState.error && <button type="button" onClick={clearOrbActionParams} className="rounded-xl border border-[color:var(--medtrak-border)] px-4 py-2 text-sm font-semibold">Dismiss</button>}
              </div>
            </div>
          </div>
        </section>
