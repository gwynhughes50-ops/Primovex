...
export default function MobileBarcodeScanner({ onScan, triggerAttribute = "data-mobile-scan-button", title = "Scan barcode", helper = "Rear camera scanning with manual fallback." }) {
  ...
  const startScanner = async () => {
    ...
    try {
      ...
      setErrText(name === "NotAllowedError" ? "Camera access is blocked. Allow camera permission for Primovex in Android settings, then retry." : `${name}: ${msg}\n\nTry better light, hold the phone still, or use manual entry.`);
      setStatus("");
      setManualMode(true);
      stopScanner();
    }
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("medtrak-mobile-lock", handleMobileLock);
      stopScanner();
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
...
>
      ...
      {open && (
        <div className="fixed inset-0 z-[150] flex items-end justify-center overflow-y-auto bg-slate-950/80 px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-[max(0.75rem,env(safe-area-inset-top))] backdrop-blur sm:items-center sm:p-4">
          <div className="max-h-[calc(100dvh-1.5rem)] w-full max-w-md overflow-y-auto rounded-[1.75rem] border border-[var(--medtrak-border)] bg-[var(--medtrak-panel)] p-4 text-[var(--medtrak-text)] shadow-2xl">
            ...
            {!manualMode && (
              <div className="relative min-h-[18rem] overflow-hidden rounded-2xl border border-[var(--medtrak-border)] bg-black">
                <video
                  ref={videoRef}
                  playsInline
                  muted
                  autoPlay
                  className="block h-[min(52vh,24rem)] min-h-[18rem] w-full object-cover"
                />
