    ...
    <div className="cf-azure-backdrop" role="presentation">
      <section className="cf-azure-panel" role="dialog" aria-modal="true" aria-labelledby="cf-azure-title" tabIndex={-1} ref={dialogRef}>
        <header>
          ...
          <div><p>ClinFlow governed batch gateway</p><h2 id="cf-azure-title">Azure Document Intelligence</h2><span>Attested anonymised documents only · documents processed individually through a controlled queue</span></div>
        ...
        <div className="cf-azure-status-row">
          ...
          <div className="cf-azure-limit"><ShieldCheck /><div><strong>Controlled batch processing</strong><span>One PDF at a time · anonymisation attested by the signed-in user · {Math.round(MAX_FILE_BYTES / 1024 / 1024)} MB per file</span></div></div>
        </div>
        ...
        {!jobs.length ? (
          <main className="cf-azure-setup cf-azure-setup--batch">
            <section className="cf-azure-upload-card">
              ...
              <p>Select one or more anonymised PDFs to test. Every file must be confirmed free of real patient or staff information before analysis. Primovex queues every selected file and processes them sequentially to respect provider capacity.</p>
              ...
              <label className="cf-azure-attestation"><input type="checkbox" checked={attested} onChange={(event) => setAttested(event.target.checked)} /><span>I confirm every selected file has been fully anonymised and contains no real patient or staff identifiable information.</span></label>
          ...
          <main className="cf-azure-batch-results">
            ...
            <section className="cf-azure-job-detail">
              ...
              {result && <>
                {result.llmExtractionError && <div className="cf-azure-error"><AlertTriangle />Clinical fact extraction did not run: {result.llmExtractionError} The raw OCR text below is still accurate — diagnosis, medication and action detection just could not complete for this document.</div>}
