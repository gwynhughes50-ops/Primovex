...
export default function NfcManager({ state, commit, actor, defaultEntityId }) {
  ...
  useEffect(() => {
    if (defaultEntityId && state.spaces.some((space) => space.id === defaultEntityId)) {
      setEntityType('space');
      setEntityId(defaultEntityId);
    }
  }, [defaultEntityId, state.spaces]);
  ...
  return (
    <section id="sense-tag-manager" className={`${panel} scroll-mt-5 p-4 sm:p-5`}>
