...
export default function AskPrimovexPanel({ variant = 'desktop' }) {
  ...
  const panelClass = variant === 'mobile'
    ? 'fixed inset-x-0 bottom-0 top-[18vh] z-[140] rounded-t-[2rem]'
    : 'fixed bottom-6 right-6 top-6 z-[140] w-[min(470px,calc(100vw-3rem))]';

  return (
    <aside className={`${panelClass} primovex-ai-panel flex flex-col overflow-hidden shadow-2xl backdrop-blur-xl ${variant === 'mobile' ? 'rounded-t-[2rem]' : 'rounded-3xl'}`} aria-label="Ask Primovex">
