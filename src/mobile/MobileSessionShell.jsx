...
const PIN_KEY = "primovex.mobile.pin.v1";
const UNLOCK_KEY = "primovex.mobile.unlocked.v1";
...
export default function MobileSessionShell({ children, showSplash = false }) {
  ...
  const [locked, setLocked] = useState(() => sessionStorage.getItem(UNLOCK_KEY) !== "yes");
  const [mode, setMode] = useState("biometric");
  const [pin, setPin] = useState("");
  const [confirmPin, setConfirmPin] = useState("");
  const [message, setMessage] = useState("");
  const [lastActivity, setLastActivity] = useState(Date.now());
  const hasPin = useMemo(() => Boolean(localStorage.getItem(PIN_KEY)), []);
      ...
      {locked && (
        <div className="fixed inset-0 z-[190] flex items-center justify-center bg-[var(--medtrak-bg)] p-5">
          <div className="w-full max-w-sm">
            ...
            <button onClick={signOut} className="mt-5 flex w-full items-center justify-center gap-2 px-4 py-3 text-sm text-[var(--medtrak-muted)]"><LogOut className="h-4 w-4" /> Use account password instead</button>
